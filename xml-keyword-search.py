...
#
# Option:       There is also an option to add ' --i' to the end of the search
#               string to do a case-insensitive search.
#
...
def tagSearch(tree, regex_compile):
    tagsDict = {}
    tagsList = []
    for tag in tree.find_all(regex_compile):
        parents_tag = []
...
def attrSearch(tree, regex_compile):
    attrDict = {}
    attrList = []
    for attr in tree.find_all(attrs={"name": regex_compile}):
        parents_attr = []
...
# and returns a list of xpaths from tag matches, and a dictionary containing each tag match along with its xpath
def stringSearch(tree, regex_compile):
    stringDict = {}
    stringList = []
    for string in tree.find_all(string=regex_compile):
        parents_string = []
    ...
    while True:
        ...
        while True:
            ...
            if keyword.endswith(' --i'):
                keyword = (keyword.strip(' --i'), True)
            else:
                keyword = (keyword, False)
            keyword_r = re.match(r"^(\s*\S+(.|\n)*)$", keyword[0])  # Check the entry for the proper format (must contain something other than just whitespace) ##
            if keyword_r:
                break
            else:
                time.sleep(1)
        ...
        if keyword[1]:
            regex_compile = re.compile(keyword[0], re.IGNORECASE)
        else:
            regex_compile = re.compile(keyword[0])
        xPathDict_tag, xPathList_tags = tagSearch(tree, regex_compile)
        xPathDict_attr, xPathList_attrs = attrSearch(tree, regex_compile)
        xPathDict_string, xPathList_strings = stringSearch(tree, regex_compile)
                    ...
                    if xpathChoice_r:
                        ...
                        if xpathChoice < count:
                            ...
                            if xpathChoice <= len(xPathList_tags):  # If the user chooses a number in the tag list ##
                                ...
                                if treeET.find(childXpath) is not None:
                                ...
                                if treeET.find('./result' + str(xPathList_attrs[xpathChoice - len(xPathList_tags) - 1])) is not None:
